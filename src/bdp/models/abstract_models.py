...
class DeepBayesianModel(torch.nn.Module):
    ...
    def _anneal_lr(self) -> None:
        if isinstance(self.lr_scheduler, list):
            for i in range(len(self.lr_scheduler)):
                if self.lr_scheduler[i]['counter'] > 0:
                    self.lr_scheduler[i]['counter'] -= 1
                else:
                    self.lr_scheduler[i]['scheduler'].step()
                    self.lr_scheduler[i]['counter'] = self.lr_scheduler[i]['default_counter']
    ...
    def _init_lr_scheduler(self, optimizer, **inference_parameters) -> Union[None, dict]:
        ...
        lr_scheduler = {'counter': counter, 'default_counter': counter, 'scheduler': opt_scheduler}
        return lr_scheduler

    def _early_stop(self) -> bool:
        lr = self.optimizer[1].param_groups[0]['lr'] if isinstance(self.optimizer, list) else self.optimizer.param_groups[0]['lr']
        return self.min_lr_rate is not None and lr < float(self.min_lr_rate)
    ...
    def initialize_inference(self, data_loader, parameters=None, **inference_parameters):
        ...
        try:
            self.optimizer = self._create_optimizer(inference_parameters.get('optimizer_name'),
                                                    self.learning_rate,
                                                    inference_parameters.get('weight_decay', 1.2 * 1e-6),
                                                    parameters=parameters)
            self.lr_scheduler = self._init_lr_scheduler(self.optimizer, **inference_parameters)
        except:
            print(sys.exc_info())
            print("Optimizer Not Created In Abstract Initialize Inference")

    def _create_optimizer(self, optmimizer_name: str, lr: float, weight_decay: float, parameters=None):
        if parameters is None:
            parameters = self.parameters()
        if optmimizer_name.lower() == "adam":
            return Adam(parameters, lr=lr, weight_decay=weight_decay, eps=1e-8)
        if optmimizer_name.lower() == "adamw":
            return AdamW(parameters, lr=lr, weight_decay=weight_decay, eps=1e-8)
                    ...
                    for epoch in tqdm(range(1, self.number_of_epochs + 1), desc="Train Epoch", unit='epoch'):
                        ...
                        if current_eval < self.best_eval:
                            ...
                            if isinstance(self.optimizer, list):
                                self.inference_results["lr"] = [opt.param_groups[0]['lr'] for opt in self.optimizer]
                                for i in range(len(self.lr_scheduler)):
                                    self.lr_scheduler[i]['counter'] = self.lr_scheduler[i]['default_counter']
                            else:
                                self.lr_scheduler['counter'] = self.lr_scheduler['default_counter']
                        ...
                        model_evaluation.update({
                            'eval_time': (datetime.now() - self.time_0).total_seconds(),
                            'epoch':
                                epoch,
                            'lr': [opt.param_groups[0]['lr'] for opt in self.optimizer]
                                  if isinstance(self.optimizer, list) else self.optimizer.param_groups[0]['lr']
                        })
    ...
    def save_model(self, end=False):
        state = {'model_name': self.model_name, 'state_dict': self.state_dict()}
        if end:
