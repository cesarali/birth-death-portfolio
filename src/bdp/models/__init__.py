...
class AModel(nn.Module, ABC):
    """Abstract model class and all new NN models_ should be sub-class of this class.

    In order one to use all feature (training, logging, ...) of the `kiwissen-base` framework
    one must implement the new models_ as sub-class of ``AModel`` class. This abstract class defines
